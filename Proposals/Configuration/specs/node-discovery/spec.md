...
Pinned by: `Tests/SiloTests/NodeTests.swift` (`anAdvertisedSiloIsFound`, skipped where no responder runs). The id and `b=1` records, the setting and its live changes, and the default name are pinned by nothing yet.
